...
public class Solution {
    public static void main(String... args) throws IOException {
        ...
        byte[] buff = new byte[text.getBytes(StandardCharsets.UTF_8).length];
        raf.seek(number);
        raf.read(buff, 0, text.getBytes(StandardCharsets.UTF_8).length);
        String fromFile = new String(buff, StandardCharsets.UTF_8);
        raf.seek(raf.length());
